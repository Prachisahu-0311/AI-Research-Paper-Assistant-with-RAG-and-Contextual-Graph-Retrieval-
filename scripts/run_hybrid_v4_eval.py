"""Run the hybrid RAG v4 eval: HyDE + decomposition + router fix."""
import json
import os
import sys
import time
...
def run_v4_eval(questions: list[dict], vs: VectorStore, judge_model: str) -> list[dict]:
...
def main() -> None:
    ...
    out_path = Path("data/eval/results_hybrid_v4.json")
    ...
    if out_path.exists():
        ...
        if not questions_to_run:
            print("All v4 questions already scored.")
            return
    ...
    confirm = input(f"Run v4 eval on {len(questions_to_run)} questions? (y/n): ").strip().lower()
    if confirm != "y":
        return

    new_results = run_v4_eval(questions_to_run, vs, judge_model)
